...
class StageProgressMapTest {
    ...
    @Test fun currentMovesThroughCharacterAndStageGate() {
        assertEquals(10, (GrowthMapModel.currentEntry(9, emptySet(), ids) as GrowthMapModel.MapEntry.CharacterNode).order)
        assertEquals(10, (GrowthMapModel.currentEntry(10, emptySet(), ids) as GrowthMapModel.MapEntry.StageTestNode).afterCharacterOrder)
        assertEquals(11, (GrowthMapModel.currentEntry(10, setOf(0), ids) as GrowthMapModel.MapEntry.CharacterNode).order)
        assertEquals(20, (GrowthMapModel.currentEntry(20, setOf(0), ids) as GrowthMapModel.MapEntry.StageTestNode).afterCharacterOrder)
        assertEquals(50, (GrowthMapModel.currentEntry(49, setOf(0, 1, 2, 3), ids) as GrowthMapModel.MapEntry.CharacterNode).order)
        assertTrue(GrowthMapModel.currentEntry(50, setOf(0, 1, 2, 3), ids) is GrowthMapModel.MapEntry.StageTestNode)
    }
}
